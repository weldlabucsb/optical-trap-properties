...
function [p]=calculateCrossedODTpotential_Sr(p)
...
p.ODTpower=3.2; % power of beam in Watts
p.ODTw=106E-6; % gaussian beam waist in m
p.XODTpower=3.6; % power of beam in Watts
p.XODTw=96E-6; % gaussian beam waist in m

% atomic parameters
p.m=84*1.67E-27; % mass of atom
p.lambda_0=461E-9;%wavelength of transition in m
p.gamma=2*pi*30.5E6;%linewidth in Radians*Hz
...
p.numpoints=301;
p.plotrangexy=4; % number of beam waists over which to calculate potential
p.plotrangez=4;  % number of rayleigh lengths over which to calculate potential
